...
We'll use the same infrastructure from the [workshop](/07-streaming/).
...
To solve this question, the [`producer_green.py`](/07-streaming/src/producers/producer_green.py) is created

Execute the `producer_green.py` file:

```bash
uv run python src/producers/producer_green.py
```

It took 8.26 seconds to process 49416 rows. The closest answer is **10 seconds**
...
```SQL
CREATE TABLE green_processed_events (
...
Then, execute [`consumer_green.py`](/07-streaming/src/consumers/consumer_green.py):

```bash
uv run python src/consumers/consumer_green.py
```

Finally, in postgres we execute the next query:

```SQL
select count(*) from green_processed_events where trip_distance > 5;
```
**Answer:** 8506
...
At first, we create the table in postgres:

```SQL
CREATE TABLE green_processed_events_aggregated (
...
Then, we execute [`aggregation_job_green.py`](/07-streaming/src/job/aggregation_job_green.py):

```bash
MSYS_NO_PATHCONV=1 docker exec -it 07-streaming-jobmanager-1 flink run -py /opt/src/job/aggregation_job_green.py
```

**Note:** MSYS_NO_PATHCONV=1 is added if you execute it on Windows, if not ignore it.

Finally, in postgres we execute the next query:

```SQL
SELECT PULocationID, num_trips
FROM green_processed_events_aggregated
ORDER BY num_trips DESC
LIMIT 3;
```
...
```SQL
CREATE TABLE green_processed_events_session (
    window_start TIMESTAMP,
...
Then, we execute [`session_window_job.py`](/07-streaming/src/job/session_window_job.py):

```bash
MSYS_NO_PATHCONV=1 docker exec -it 07-streaming-jobmanager-1 flink run -py /opt/src/job/session_window_job.py
```
...
FROM green_processed_events_session
ORDER BY num_trips DESC
LIMIT 3;
```
...
```SQL
CREATE TABLE green_tumbling_window (
    window_start TIMESTAMP,
    window_end TIMESTAMP,
...
Then, we execute [`tumbling_window_job.py`](/07-streaming/src/job/tumbling_window_job.py):

```bash
MSYS_NO_PATHCONV=1 docker exec -it 07-streaming-jobmanager-1 flink run -py /opt/src/job/tumbling_window_job.py
```
